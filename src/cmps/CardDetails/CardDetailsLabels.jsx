    ...
    return (
        <div className="card-details-labels item-container flex column align-flex-end">
            ...
            <div className="labels-container flex wrap">
                {labels.map(label => {
                    return <span onClick={(ev) => onOpenPopover(ev, 'LABELS')} key={label.id} className="label" style={{ backgroundColor: label.color }}>
                        {label.title}
                    </span>
                })}
                <button className="secondary-btn" onClick={(ev) => onOpenPopover(ev, 'LABELS')} ><AddIcon /></button>
            </div>
        </div>
    )
}