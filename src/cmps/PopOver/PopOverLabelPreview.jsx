...
export function PopoverLabelPreview({ label, toggleLabel, isSelected, toggleEditMode,isFilterPreview }) {
    ...
    return <li className="flex">
        {!isFilterPreview?
        <> 
        <div style={{ backgroundColor: label.color, color: 'white' }} name="label" onClick={() => toggleLabel(label,'labels')} className="label-pop-over-preview flex justify-space-between">
        ...
        :
        <div className="filter-label-preview flex align-center" onClick={() => toggleLabel(label,'labels')}>
